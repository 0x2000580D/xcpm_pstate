...
int main(int argc, const char * argv[]) {
    ...
    pstate_table *table = malloc(sizeof(pstate_table));
    ioctl(fd, XCPM_GET_PSTATE_TABLE, table, 0);
    
    uint32_t pstatecnt = table->state_count;
    
    uint64_t* ctr_a = malloc(sizeof(uint64_t) * pstatecnt);
    uint64_t* ctr_b = malloc(sizeof(uint64_t) * pstatecnt);
